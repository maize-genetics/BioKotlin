...
//
//
////TODO add ability to get NucSeq from attached FASTA (or throw exception if no attached FASTA)
//
///**
...
//    /*
//    Everything that implements this interface (except FeatureImpl & AncestralFeature) should also have its own version
//    of the following functions, which cannot be part of the interface either because they are dangerous to expose,
//    due not make sense for FeatureImpl, or impose specific type constraints.
//    1. mutableSubtree()
//    2. immutableSubtree()
//    3. mutable()
//    4. immutable()
...
//    public override var strand: Char? //TODO change should be propagated down & up -- document this!
...
//
//    /*
//    TODO potentially reconsider ordering invariants for mutable implementations.
//     Ordering offers minimal performance improvements and mutations
//     of seqid, start, end, and type will force reordering, which may worsen performance.
//     Also, cannot be sure that flatten will follow ordering because start-end bounds are allowed
//     to extend past parents :(
//     */
//
//    /**
//     * INVARIANTS:
...
//        if (start < 1) throw IllegalStateException("Start must be positive but was $start")
...
//    public override fun genome(): Genome {
//        var current: Ancestor = parent
//        while (current !is Genome) {
//            current = (current as Feature).parent // All non-genome ancestors are features
//        }
//        return current
//    }
//
...
//     */
//    internal fun mutable(feature: Feature): MutableFeature = (feature.genome() as GenomeImpl).mutable(feature)
//
//    /**
//     * Creates a deeply immutable clone of the entire tree that [feature] is in, then returns the clone that corresponds
//     * to [feature] in the new tree
//     */
//    internal fun immutable(feature: Feature): Feature = (feature.genome() as GenomeImpl).immutable(feature)
//
//            /**
...
//
//    public override fun genome(): MutableGenome = super.genome() as MutableGenome // safe cast due to no mutability mixing invariant
//
...
//
//    /**
//     * Sets the parent to [newParent]. Makes no requirements about the initialization of the backing property (unlike
//     * [injectParent])
//     */
